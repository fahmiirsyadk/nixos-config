...
with lib;
let cfg = config.programs.fahmi.bash;
in {
  options.programs.fahmi.bash = {
  ...
  config = mkIf cfg.enable {
    ...
    fonts.fonts = (optionals cfg.starship.enable
    ...
    nixpkgs.overlays = [
      (self: super: {
        nix-direnv = super.nix-direnv.override { enableFlakes = false; };
      })
    ];

    environment.etc.bashrc.text = if !cfg.starship.enable then
